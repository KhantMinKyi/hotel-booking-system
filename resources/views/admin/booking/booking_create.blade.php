...
@section('content')
    <div class="flex justify-center text-lg bg-gray-100 pt-2 font-bold">
        <div
            ...
            <form action="{{ route('admin_room_booking.store') }}" method="POST">
                @csrf
                <input type="hidden" name="booking_user_name" id="booking_user_name" value="">
                <input type="hidden" name="booking_user_phone" id="booking_user_phone" value="">
    ...
    <script>
        ...
        $(document).on('change', '#user_name', function() {
            var name = $(this).val();
            $('#booking_user_name').val(name);
        })
        $(document).on('change', '#user_phone', function() {
            var phone = $(this).val();
            $('#booking_user_phone').val(phone);
        })
    </script>
@endsection
